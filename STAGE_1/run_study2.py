#!/usr/bin/env python3
"""
run_study2.py — Study 2: Multi-init Phase 1 vs uniform-random baseline.

Two conditions, both using all_fixes hyperparameters from Study 1:
  baseline    uniform-random Phase 1 (current default)
  multi_init  try all 8 Phase-1 strategies, keep the one closest to x_orig

Within multi_init we also record which strategy wins each image, so the
study answers two questions at once:
  1. Does multi-init Phase 1 improve final L2 overall?
  2. Which individual strategies contribute most?

Usage:
    python run_study2.py           # full run (N=200, Q=2000)
    python run_study2.py --mock    # quick check (N=10, Q=300)
"""
...
from phase1_zoo    import INIT_ZOO, make_multi_init
...
OUTPUT_DIR   = f'outputs/study2_{TAG.lower()}'
...
# All-fixes hyperparameters (best from Study 1 motivation)
HPARAMS = dict(xi_correction=True, bs_steps=15, tau=1,
               lam_override=None, cmu_scale=0.1)
...
log(f'Max queries: {MAX_Q}  |  Hyperparams: {HPARAMS}')
...
if len(mi):
    ...
    init_cols = [c for c in df.columns if c.startswith('init_') and c.endswith('_l2')
                 and c not in ('init_winner_l2',)]
    for mname in MODEL_NAMES:
        sub = mi[mi.model == mname]
...
for ax, mname in zip(axes, MODEL_NAMES):
    ...
    ax.set_title(mname); ax.grid(axis='y', alpha=0.3)
plt.suptitle('B: Final L2  —  does better Phase 1 survive into Phase 3?', fontsize=12)
plt.tight_layout()
